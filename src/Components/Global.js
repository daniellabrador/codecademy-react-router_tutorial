...
function Global(props) {
  ...
  useEffect(()=>{
    ...
    const fetchItems = async () => {
      try {
        const endpoint = 'https://api.spotify.com/v1/browse/new-releases?offset=0&limit=50';
  ...
  return (
    <div className="featured">
      <h2> 
        New Releases
        <br />
        <span className="uppercase" style={{fontSize: '.625em', color: '#1ED760', top: '-.5em', position: 'relative'}}> Global Hits</span>
      </h2>
      <ul className="albums">
        {albums.map(album=>{
          return (
